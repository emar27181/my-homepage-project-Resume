...
- スマホ幅(560px以下)では、物理キーボードにない`Tab`と、確実に効く`Enter`をオンスクリーンのボタンとして入力欄の下に表示する。さらにその下に`ls`/`cd`/`cat`/`tree`/`open`/`whoami`/`help`のクイック起動ボタンを表示し、フルで打たなくても主要なコマンドをタップで使えるようにしている(`cd`/`cat`/`open`は引数が要るため、タップすると入力欄に`cmd `まで入って続きを打てる状態になる)。
