...
- `/v3` (v3): 研究者ポートフォリオ風のデザイン。[emar27181/portfolio-taraba](https://github.com/emar27181/portfolio-taraba)というAstro製テーマを、このサイト自身のプロフィール・制作物・研究発表・スキル・経歴のデータで動くように移植したもの。サイドバー目次・ライト/ダーク切り替え・引用情報コピーなどの操作感を持つ1ページ構成。
...
`/v3`はサイドバーの目次から各セクション(About / Research Interests / Featured Projects / Publications / Presentations / Skills / Education-History / Contact)へジャンプできる1枚の縦スクロールページ。データが無いセクション(移植元にあるAwards等)は自動的に表示されない。ヘッダーの丸ボタンでライト/ダーク切り替え、Publications/Presentationsの各項目では引用情報をクリップボードにコピーできる。コンテンツはv2と同じ`src/v2/data/portfolio.ts`から`src/v3/data/adapter.ts`が組み替えて表示しているだけで、v3専用のコンテンツファイルは無い(内容を直すときはv2側のファイルを編集する)。
