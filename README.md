...
- `/v3` (v3): 研究者ポートフォリオ風のデザイン。[emar27181/portfolio-taraba](https://github.com/emar27181/portfolio-taraba)というAstro製テーマを、このサイト自身の研究データ(研究テーマ・学会発表)で動くように移植したもの。制作物・スキル・学歴などはv1で既に見られるため、v3では研究関連のセクションだけを表示する。v1と同じ「スクロール連動タブ+スティッキーヘッダー」の操作感、ライト/ダーク切り替え、引用情報コピーを持つ1ページ構成。
