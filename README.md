...
- スマホ幅(560px以下)では、物理キーボードにない`Tab`・`↑`/`↓`(コマンド履歴)・`←`/`→`(カーソル移動)・確実に効く`Enter`・`Ctrl+L`相当の`Clear`をオンスクリーンのボタンとして入力欄の下に表示する。これらは実際のキー操作と同じくタップした瞬間に実行される。さらにその下に`ls`/`cd`/`cat`/`tree`/`open`/`whoami`/`help`のクイック起動ボタンを表示し、フルで打たなくても主要なコマンドをタップで使えるようにしている。こちらはタップするのは補完だけで、実行はしない(Tab補完と同じ扱い)。`cd`/`cat`/`open`は入力欄に`cmd `まで入り、`ls`等の引数無しコマンドはそのままの文字列が入る。実行するには続けて`Enter`(ボタンまたはキーボード)を押す必要がある。
